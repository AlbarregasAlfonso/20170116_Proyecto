 ...
 */
@WebServlet(name = "ControllersComprandoIniciandoSesion", urlPatterns = {"/ControllersComprandoIniciandoSesion"})
public class ControllersComprandoIniciandoSesion extends HttpServlet {
            ...
            if (request.getParameter("Enviar") != null) {
                ...
                if (request.getParameter("Enviar").equals("registro")) {
                    ...
                    if (udao.userNameyautilizado(request.getParameter("user")).equals("OK") && udao.emailyautilizado(request.getParameter("email")).equals("OK")) {
                        ...
                        Pedidos p = new Pedidos(udao.getSacarIdUsuario(request.getParameter("user")), "p");
                        pedao.insertarCarrito(p);

                        lpdao.idLineaPedidoMax();
                        int numeroLinea = lpdao.idLineaPedidoMax();
                        LineasPedidos lp = new LineasPedidos(pedao.idPedidoMaxDeUnCliente(udao.getSacarIdUsuario(request.getParameter("user"))), numeroLinea + 1, request.getParameter("idproducto"), request.getParameter("cantidad"));
                    ...
                    if (udao.inicioSessionConMail2(request.getParameter("user"), request.getParameter("clave")).equals("usuario bloqueado") || udao.inicioSessionConMail2(request.getParameter("user"), request.getParameter("clave")).equals("Usuario o contraseña erroneos")) {
                        ...
                        if (request.getParameter("productoCaracteristicas") != null) {
                            ILineasPedidosDAO lpdao = daof.getLineaPedidosDAO();
                            IPedidosDAO pedao = daof.getPedidosDAO();
                            ...
                            if (lpdao.buscarProductoEnLineaPedidos(u.getIdUsuario(), request.getParameter("idproducto"))) {
                                lpdao.insertarProductoACarrito(lp);
                                request.setAttribute("mensaje", "Nuevo carrito creado");
                            } else {
                                request.setAttribute("mensaje", "Este producto ya estaba en carrito");
                            }
